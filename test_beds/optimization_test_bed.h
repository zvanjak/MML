...
namespace MML::TestBeds
{
    ...
    inline TestOptimization1D getLogBarrier1DTest()
    {
        return TestOptimization1D{
            ...
            0.5, 2.0,  // Tighter interval containing minimum, avoids domain issues
            "unimodal",
    ...
    /**
     * @brief Get all 1D optimization test cases suitable for unconstrained optimization
     * 
     * Note: Functions with domain constraints (like Log Barrier) are excluded
     * since bracket expansion may violate domain constraints.
     * Use getConstrainedDomain1DTests() for those functions.
     */
    inline std::vector<TestOptimization1D> getAll1DOptimizationTests()
    {
        return {
            ...
            getCosh1DTest(),
            getAbsValue1DTest(),
            // LogBarrier excluded - domain constraint x > 0
            getRastrigin1DTest(),
            getAckley1DTest(),
            getSinusoidal1DTest()
        };
    ...
    {
        // Note: Log Barrier (x² - ln(x)) is excluded because it has domain constraint (x > 0)
        // and bracket expansion can violate this constraint, causing NaN.
        // Use getConstrainedDomain1DTests() for functions with domain restrictions.
        return {
            getQuadratic1DTest(),
            getQuartic1DTest(),
            getCosh1DTest(),
    ...
    /**
     * @brief Get 1D test cases with constrained domains
     * 
     * These functions have domain restrictions that standard unconstrained
     * optimization methods may violate during bracket expansion.
     */
    inline std::vector<TestOptimization1D> getConstrainedDomain1DTests()
    {
        return {
            getLogBarrier1DTest()  // Domain: x > 0
        };
    }
