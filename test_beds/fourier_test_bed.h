...
namespace MML::TestBeds {
	...
	inline TestSignal getDualToneSignal() {
		...
		for (int i = 0; i < N; i++) {
			Real t = i / sampleRate;
			test.data[i] = amp1 * std::sin(2.0 * Constants::PI * freq1 * t) + amp2 * std::sin(2.0 * Constants::PI * freq2 * t);
			energy += test.data[i] * test.data[i];
		}
	...
	inline std::vector<TestSignal> getAllSignals() {
		return {getPureToneSignal(),		getCosineToneSignal(),			 getDualToneSignal(),			getHarmonicSignal(),
						getLinearChirpSignal(), getExponentialChirpSignal(), getImpulseSignal(),			getStepSignal(),
						getWhiteNoiseSignal(),	getRectangularPulseSignal(), getGaussianPulseSignal()};
	}
     ...
     */
	inline std::vector<ConvolutionTest> getAllConvolutionTests() { return {getSimpleConvolutionTest(), getImpulseResponseTest()}; }

	/**
     * @brief Get all correlation tests
     */
	inline std::vector<CorrelationTest> getAllCorrelationTests() {
		return {getPeriodicAutoCorrelationTest(), getDelayedCrossCorrelationTest()};
	}
