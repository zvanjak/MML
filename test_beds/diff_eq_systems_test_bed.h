...
namespace MML::TestBeds {
	...
	public:
		...
		TestODESystemWithSolutionWrapper(Vector<Real> icLow, Vector<Real> icHigh, Args&&... args)
				: _odeSys(std::forward<Args>(args)...)
				, _initCondLow(icLow)
				, _initCondHigh(icHigh) {}
		...
		Vector<Real> getSolution(const Vector<Real>& initCond, Real t) const override { return _odeSys.getSolution(initCond, t); }
	...
	public:
		...
		TestODESystemWithEndSolution(int n, void (*inFunc)(Real, const Vector<Real>&, Vector<Real>&), Vector<Real> init_cond, Real start_time,
																 Real end_time, Vector<Real> end_solution)
				: _ownedOdeSystem(std::make_shared<ODESystem>(n, inFunc))
				, _odeSystem(_ownedOdeSystem.get())
				, _initial_conditions(init_cond)
				, _start_time(start_time)
				, _end_time(end_time)
				, _end_solution(end_solution) {}

		// Constructor with shared_ptr to IODESystem
		TestODESystemWithEndSolution(std::shared_ptr<IODESystem> odeSystem, Vector<Real> init_cond, Real start_time, Real end_time,
		...
		TestODESystemWithEndSolution(const IODESystem* odeSystem, Vector<Real> init_cond, Real start_time, Real end_time,
																 Vector<Real> end_solution)
		...
		TestODESystemWithEndSolution(int n, void (*inFunc)(Real, const Vector<Real>&, Vector<Real>&), Vector<Real> init_cond, Real end_time,
																 Vector<Real> end_solution)
				: TestODESystemWithEndSolution(n, inFunc, init_cond, 0.0, end_time, end_solution) {}

		TestODESystemWithEndSolution(const IODESystem* odeSystem, Vector<Real> init_cond, Real end_time, Vector<Real> end_solution)
	...
	private:
		...
		static inline std::vector<ODESystemWithSolutionEntry> _systemsWithSolution = {
			// Linear systems
			{"ExpDecay_k1", ODECategory::Linear, &_expDecay, [](const Vector<Real>& y0, Real t) { return _expDecay.getSolution(y0, t); },
			 Vector<Real>{0.1}, Vector<Real>{10.0}},

			{"ExpGrowth_k0.5", ODECategory::Linear, &_expGrowth, [](const Vector<Real>& y0, Real t) { return _expGrowth.getSolution(y0, t); },
			 Vector<Real>{0.1}, Vector<Real>{5.0}},

			{"Linear2D_DistinctEigen", ODECategory::Linear, &_linear2D,
			 [](const Vector<Real>& y0, Real t) { return _linear2D.getSolution(y0, t); }, Vector<Real>{-5.0, -5.0}, Vector<Real>{5.0, 5.0}},

			// Oscillators
			{"SHO_omega1", ODECategory::Oscillatory, &_sho, [](const Vector<Real>& y0, Real t) { return _sho.getSolution(y0, t); },
			 Vector<Real>{0.0, -10.0}, Vector<Real>{Constants::PI, 10.0}},

			{"SHO_omega2", ODECategory::Oscillatory, &_shoOmega2, [](const Vector<Real>& y0, Real t) { return _shoOmega2.getSolution(y0, t); },
			 Vector<Real>{0.0, -10.0}, Vector<Real>{Constants::PI, 10.0}},

			{"DampedSHO_zeta0.1", ODECategory::Oscillatory, &_dampedSHO,
			 [](const Vector<Real>& y0, Real t) { return _dampedSHO.getSolution(y0, t); }, Vector<Real>{0.0, -10.0},
			 Vector<Real>{Constants::PI, 10.0}},

			// Population dynamics
			{"Logistic_r1_K10", ODECategory::Population, &_logistic, [](const Vector<Real>& y0, Real t) { return _logistic.getSolution(y0, t); },
		...
		static inline std::vector<ODESystemWithEndSolutionEntry> _systemsWithEndSolution = {
			...
			{"ExpDecay_t5", ODECategory::Linear, &_expDecay, Vector<Real>{1.0}, 0.0, 5.0, _expDecay.getSolution(Vector<Real>{1.0}, 5.0)},

			{"ExpGrowth_t2", ODECategory::Linear, &_expGrowth, Vector<Real>{1.0}, 0.0, 2.0, _expGrowth.getSolution(Vector<Real>{1.0}, 2.0)},

			{"Linear2D_t3", ODECategory::Linear, &_linear2D, Vector<Real>{1.0, 3.0}, 0.0, 3.0,
			 _linear2D.getSolution(Vector<Real>{1.0, 3.0}, 3.0)},

			{"Linear3D_t1", ODECategory::Linear, &_linear3D, Vector<Real>{1.0, 1.0, 1.0}, 0.0, 1.0, _linear3D.getSolutionForUnitIC(1.0)},

			// Oscillators (multiple periods)
			{"SHO_1period", ODECategory::Oscillatory, &_sho, Vector<Real>{1.0, 0.0}, 0.0, 2 * Constants::PI,
			 _sho.getSolution(Vector<Real>{1.0, 0.0}, 2 * Constants::PI)},

			{"SHO_5periods", ODECategory::Oscillatory, &_sho, Vector<Real>{1.0, 0.0}, 0.0, 10 * Constants::PI,
			 _sho.getSolution(Vector<Real>{1.0, 0.0}, 10 * Constants::PI)},

			{"SHO_omega2_2periods", ODECategory::Oscillatory, &_shoOmega2, Vector<Real>{1.0, 0.0}, 0.0, 2 * Constants::PI,
			 _shoOmega2.getSolution(Vector<Real>{1.0, 0.0}, 2 * Constants::PI)},

			{"DampedSHO_t10", ODECategory::Oscillatory, &_dampedSHO, Vector<Real>{1.0, 0.0}, 0.0, 10.0,
			 _dampedSHO.getSolution(Vector<Real>{1.0, 0.0}, 10.0)},

			{"DampedSHO_t20", ODECategory::Oscillatory, &_dampedSHO, Vector<Real>{1.0, 0.0}, 0.0, 20.0,
			 _dampedSHO.getSolution(Vector<Real>{1.0, 0.0}, 20.0)},

			// Population dynamics
			{"Logistic_t5", ODECategory::Population, &_logistic, Vector<Real>{1.0}, 0.0, 5.0, _logistic.getSolution(Vector<Real>{1.0}, 5.0)},

			{"Logistic_t10", ODECategory::Population, &_logistic, Vector<Real>{0.5}, 0.0, 10.0, _logistic.getSolution(Vector<Real>{0.5}, 10.0)},

			// Legacy compatibility
			{"TestLinODESys_t1", ODECategory::Linear, &_linear3D, Vector<Real>{1.0, 1.0, 1.0}, 0.0, 1.0, TestLinODESys_sol(1.0)},

			{"TestLinODESys_t5", ODECategory::Linear, &_linear3D, Vector<Real>{1.0, 1.0, 1.0}, 0.0, 5.0, TestLinODESys_sol(5.0)}};
		...
		static inline std::vector<std::pair<std::string, const IODESystemWithJacobian*>> _systemsWithJacobian = {{"StiffExample1", &_stiff1}};
	...
	public:
		...
		static const IODESystem* getSystemWithSolution(int index) { return _systemsWithSolution[index].system; }

		static Vector<Real> getSolution(int index, const Vector<Real>& ic, Real t) { return _systemsWithSolution[index].solution(ic, t); }

		static const ODESystemWithEndSolutionEntry& getSystemWithEndSolution(int index) { return _systemsWithEndSolution[index]; }

		static const ODESystemEntry& getGeneralSystem(int index) { return _generalSystems[index]; }

		static const IODESystemWithJacobian* getSystemWithJacobian(int index) { return _systemsWithJacobian[index].second; }
		...
		static std::vector<const ODESystemWithEndSolutionEntry*> getSystemsWithEndSolutionByDimension(int dim) {
			return filterSystemsWithEndSolution([dim](const ODESystemWithEndSolutionEntry& e) { return e.system->getDim() == dim; });
		}
		...
		static std::vector<const ODESystemEntry*> getChaoticSystems() { return getGeneralSystemsByCategory(ODECategory::Chaotic); }

		//--------------------------------------------------------------------------------------------------------------
		// CONVENIENCE: GET ALL 1D SYSTEMS (simplest tests)
		//--------------------------------------------------------------------------------------------------------------

		static std::vector<const ODESystemWithEndSolutionEntry*> get1DSystems() { return getSystemsWithEndSolutionByDimension(1); }

		//--------------------------------------------------------------------------------------------------------------
		// CONVENIENCE: GET ALL 2D SYSTEMS
		//--------------------------------------------------------------------------------------------------------------

		static std::vector<const ODESystemWithEndSolutionEntry*> get2DSystems() { return getSystemsWithEndSolutionByDimension(2); }
		...
		static inline ODESystem _legacySystems[3] = {
			ODESystem{3, TestLinODESys}, ODESystem{2, VanDerPolMju0_1},
		...
		static int numODESystemWithJacobian() {
			return 1;
		}

		[[deprecated("Use numSystemsWithSolution() instead")]]
		static int numODESystemWithSolution() {
			return numSystemsWithSolution();
		}

		[[deprecated("Use numSystemsWithEndSolution() instead")]]
		static int numODESystemWithEndSolution() {
			return 4;
		}
