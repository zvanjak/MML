...
namespace MML::Tests::Statistics::ConfidenceIntervalsTests
{
	...
	TEST_CASE("Statistics - ConfidenceIntervalMeanDetailed values match simple API", "[statistics][confidence][Detailed]")
	{
		...
		REQUIRE_THAT(detailed.estimate, WithinAbs(simple.estimate, 1e-15));
		REQUIRE_THAT(detailed.lowerBound, WithinAbs(simple.lowerBound, 1e-15));
		REQUIRE_THAT(detailed.upperBound, WithinAbs(simple.upperBound, 1e-15));
		REQUIRE_THAT(detailed.marginOfError, WithinAbs(simple.marginOfError, 1e-15));
		REQUIRE_THAT(detailed.confidenceLevel, WithinAbs(simple.confidenceLevel, 1e-15));
