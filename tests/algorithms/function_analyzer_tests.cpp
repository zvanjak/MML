...
TEST_CASE("Test_Function_Analyzer_FuncDiff", "[simple]") 
{
    ...
    REQUIRE( FunctionAnalyzer::FuncDiff(myfunc, test, 0.0, 6.0) == Approx(-9.0).epsilon(1e-6) );
    REQUIRE( FunctionAnalyzer::FuncDiff(myfunc, test, 0.0, 6.0) != Approx(-9.0).epsilon(1e-7) );
