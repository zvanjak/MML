...
#include "mml/core/CoordSystem.h"
...
void Chapter12_artillery_shell_simulator()
{

}

// takes into account changing mass of the rocket!
void Chapter12_rocket_simulator()
{
}
...
{
  std::cout << "***********************************************************************" << std::endl;
  std::cout << "****                   EXAMPLE 12 - projectile launch               ****" << std::endl;
  std::cout << "***********************************************************************" << std::endl;

  InertialFrame3D SolarSystemCMFrame(nullptr, Vec3Cart(0, 0, 0), Vec3Cart(0, 0, 0));

  Real earthOrbitPeriod = 365 * 24 * 3600;
  CircleOrbitingFrame3DCartesian EarthCMFrame(&SolarSystemCMFrame, 150000, earthOrbitPeriod);    // radijus kru�enja (jedinica 10^6 m, 1000km), period kru�enja (s)
  ...
  Real h = REAL(0.0);
  Real lat = REAL(Utils::ExplicitToAngleDeg(45, 48, 47.4)); // 45°48'47.4"N
  Real lon = REAL(Utils::ExplicitToAngleDeg(15, 58, 38.3)); // 15°58'38.3"E

  Real T = 0;
  Real V = 10; // m/s 50, 100, 300, 500, 1000, 5000
  ...
  Real one_lat_deg_in_km = REAL(2 * 6400 * cos(Utils::DegToRad(45)) * Constants::PI / 360);
  Real one_km_in_long_deg = REAL(1) / one_lat_deg_in_km;
  // transformirati lokalni (x, y, z) u parent sferni (lat, lon, h)

  // najprije kalkulacije u lokalnom frameu, sve dok nije preko 1. kozmicke brzine
