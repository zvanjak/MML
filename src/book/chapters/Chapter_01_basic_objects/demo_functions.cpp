...
Real Demo_Functions_RealFunc(Real x) {
  return sin(x) * (1 + x * x / 2);
}

// simple scalar function, returning 1 / r, for given point
Real Demo_Functions_ScalarFunc(const VectorN<Real, 3>& x) {
  return 1 / x.NormL2();
}

// rotating vector field in 2D
VectorN<Real, 2> Demo_Functions_VectorFunc(const VectorN<Real, 2>& x) {
  return VectorN<Real, 2>{-x[1], x[0]};
}

// helix curve in 3D
VectorN<Real, 3> Demo_Functions_ParamCurve(Real t) {
  return  VectorN<Real, 3>{ cos(t), sin(t), t};
}

// simple surface
VectorN<Real, 3> Demo_Functions_ParamSurface(Real x, Real y) {
  return VectorN<Real, 3>{x* y, 2 * x * y, 3 * x};
}

// little bit more realistic scalar function :)
Real Demo_Functions_two_masses_gravity_potential(const VectorN<Real, 3>& x)
{
...
void Demo_Functions_case_1_usage()
{
  // creating a function objects from an already existing (standalone) function
  RealFunction            fReal(Demo_Functions_RealFunc);
  ScalarFunction<3>       fScalar(Demo_Functions_ScalarFunc);
  ScalarFunction<3>       fScalar2(Demo_Functions_two_masses_gravity_potential);
  VectorFunction<2>       fVector(Demo_Functions_VectorFunc);
  ParametricCurve<3>      paramCurve(Demo_Functions_ParamCurve);
  ParametricSurfaceRect<3>    paramSurface(Demo_Functions_ParamSurface);
}

/////////////////////////////////////////////////////////////////////////////////////
// CASE 2 - creating function directly from lambda

void Demo_Functions_case_2_usage()
{
  std::cout << "**********************************************************************" << std::endl;
...
void Demo_Functions_case_3_usage()
{
  std::cout << "**********************************************************************" << std::endl;
...
void Demo_Functions_case_4_usage()
{
  std::cout << "**********************************************************************" << std::endl;
...
void Demo_Functions_case_5_usage()
{
  std::cout << "**********************************************************************" << std::endl;
...
void Demo_Functions()
{
  Demo_Functions_case_1_usage();
  Demo_Functions_case_2_usage();
  Demo_Functions_case_3_usage();
  Demo_Functions_case_4_usage();
  Demo_Functions_case_5_usage();
}