...
void Example20_Diff_geometry_curves()
{
	// testing curves
	Circle3DXYCurve circle(1.0);

	auto tang = circle.getTangent(1.0);
	std::cout << "Tangent at t=1.0: " << tang << std::endl;
}

void Example20_Diff_geometry_surfaces()
{
	// testing surfaces
	PlaneSurface	plane({ 0, 0, 0 }, { 1, 0, 0 }, { 0, 1, 0 });
	CylinderSurface			cylinder(2.0, 10.0);
...
void Example20_Diff_geometry_curves_surfaces()
{
	std::cout << "***********************************************************************" << std::endl;
	std::cout << "****                   EXAMPLE 20 - Curves and surfaces            ****" << std::endl;
	std::cout << "***********************************************************************" << std::endl;

	Example20_Diff_geometry_curves();
	Example20_Diff_geometry_surfaces();
}