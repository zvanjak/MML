...
void Matrix_init()
{
	...
	Matrix<Real>    unit_r = Matrix<Real>::Identity(3);
	Matrix<Complex> unit_c = Matrix<Complex>::Identity(2);
...
void Matrix_operations()
{
	...
	Vector<Real> v1({ 1.0, 2.0, 3.0, 4.0 }), v2({ 5.0, -1.0, 2.0, 3.0 });

	// scalar results
	Real n1 = m1.NormL1();		// L1 norm of matrix m1
	Real tr = m1.Trace();			// trace of matrix m1

	// vector results
	Vector<Real>  v = m1 * v1;			// matrix - vector multiplication

	// matrix results
	Matrix<Real> m3 = m1 * m2;			// matrix - matrix multiplication
	Matrix<Real> m4 = 2.5 * m1 + m2.GetSubmatrix(0, 0, 2, 3).transpose() / 1.5; 
}
