...
namespace MPL
{
	...
	public:
		...
		/******************************************************************************
		 * Config4: STAR CLUSTER COLLISION (THE SPECTACULAR ONE!)
		 * 
		 * Two separate star clusters approach and swing past each other!
		 * Each cluster has:
		 *   - A central massive body (black hole / dense core)
		 *   - N stars in bound orbits around it
		 * 
		 * Watch gravitational dynamics unfold as they interact!
		 *****************************************************************************/
		static NBodyGravitySimConfig Config4_StarClusterCollision(
			int bodiesPerCluster = 100, 
			Real separation = 800.0,
			Real impactParameter = 250.0,  // Vertical offset - clusters pass by, not head-on!
			Real approachSpeed = 5.0, 
			Real G = 1.0)
		{
			NBodyGravitySimConfig config(G);

			// Helper lambda: Generate a cluster at given offset with bulk velocity
			auto generateCluster = [&](Vec3Cart offset, Vec3Cart bulkVel, std::string starColor, Real centralMass, Real centralRadius) {
				...
				for (int i = 0; i < bodiesPerCluster; i++) {
					...
					Vec3Cart localPos{r * sin(phi) * cos(theta), r * sin(phi) * sin(theta), r * cos(phi)};
	...
	public:
		...
		// Convenience methods for energy conservation checks
		Real getInitialEnergy() const
		{
			return _vecStates[0].TotalKineticEnergy() + _vecStates[0].TotalPotentialEnergy();
		}
		Real getFinalEnergy() const
		{
			int last = static_cast<int>(_vecStates.size()) - 1;
			return _vecStates[last].TotalKineticEnergy() + _vecStates[last].TotalPotentialEnergy();
		}
		Real getRelativeEnergyError() const
		{
			Real initial = getInitialEnergy();
			Real final_energy = getFinalEnergy();
			return (initial != 0) ? (final_energy - initial) / std::abs(initial) : 0;
		}
