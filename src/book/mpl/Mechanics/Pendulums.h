...
namespace MPL
{
	...
	public:
		...
		double calcExactPeriod(double initialAngle)
		{
			...
			// Use MML wrapper if available, otherwise use arithmetic-geometric mean (AGM) algorithm
#if MML_HAS_STD_SPECIAL_FUNCTIONS
			double ellipticIntegral = std::comp_ellint_1(k);
#else
			// AGM algorithm for K(k) = π/(2*AGM(1, sqrt(1-k²)))
			double a = 1.0;
			...
			double ellipticIntegral = Constants::PI / (2.0 * a);
#endif
