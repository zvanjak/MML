...
void Readme_Real_function_visualization()
{
    ...
    Visualizer::VisualizeRealFunction(f1, "readme_real_func1" , -10.0, 10.0, 500, "readme_real_func1.mml");

    RealFuncDerived4 f2(f1);        // derivation of f1 function

    Visualizer::VisualizeRealFunction(f2, "readme_real_func2", -10.0, 10.0, 100, "readme_real_func2.mml");
    
    // shown together
    Visualizer::VisualizeMultiRealFunction({&f1, &f2}, "readme_multi_real_func", {"Func 1", "Func 2"},
                                            -10.0, 10.0, 500, "readme_multi_real_func.mml" );
}
...
void Readme_Scalar_function_2D_visualization()
{
    ...
    Visualizer::VisualizeScalarFunc2DCartesian(testFunc1, "readme_surface1" , -10.0, 10.0, 20, -10.0, 10.0, 20, "readme_surface1.mml");

    ScalarFunction<2> testFunc2{[](const VectorN<Real, 2> &x) { return (std::abs(x[0])-10) * (std::abs(x[1])-10) * sin(x[0]) * cos(x[1]); } };

    Visualizer::VisualizeScalarFunc2DCartesian(testFunc2, "readme_surface2" , -10.0, 10.0, 50, -10.0, 10.0, 50, "readme_surface2.mml");
}
...
//     testFunc.Serialize3DCartesian(-10.0, 10.0, 10, -10.0, 10.0, 10, -10.0, 10.0, 10, "func_cart_3d.mml");
// }
...
void Readme_Parametric_curve_visualization()
{
    ...
    Visualizer::VisualizeParamCurve3D(helix, "helix" , -50.0, 50.0, 1000, "readme_curve_helix.mml");
    Visualizer::VisualizeParamCurve3D(toroid, "toroid", 0.0, 2 * Constants::PI, 5000, "readme_curve_toroid.mml");
}
...
void Readme_Vector_field_visualization()
{
    ...
    Visualizer::VisualizeVectorField3DCartesian(gravity_force_field, "gravity_force_field" , -200.0, 200.0, 30, -200.0, 200.0, 30, -200.0, 200.0, 30, "readme_vector_field.mml");
}

void Readme_visualizators()
{
    std::cout << "***********************************************************************" << std::endl;
    std::cout << "****                   README - visualizators         xhdldlj             ****" << std::endl;
    std::cout << "***********************************************************************" << std::endl;
