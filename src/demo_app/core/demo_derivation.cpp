...
void Demo_Derivation_func_ptr()
{
	...
	Real num_der4 = Derivation::Derive(f1, 0.5);
}
...
void Demo_Derivation_member_fun2(const BigComplexClassYouCantChange& ref)
{
	...
	Real der_2 = Derivation::Derive(func_to_derive, 0.5);
}
...
void Demo_Derivation_member_fun3(const BigComplexClassYouCantChange& ref)
{
	...
	Real der_2 = Derivation::Derive(funcObj, 0.5);
}
...
void Demo_Derivation_member_fun4(const BigComplexClassYouCantChange& ref)
{
	...
	Real der_2 = Derivation::Derive(func_to_derive2, 0.5);
}
...
void Demo_Derivation_Interpolated_RealFunc()
{
	...
	Real der_f6 = Derivation::Derive(f_linear, 0.5);
}
...
void Demo_Derivation_Scalar_func_second()
{
	...
	// Second partial derivatives - using optimized direct formulas (3-5 function evaluations)
	// Pure second partials: ∂²f/∂xᵢ²
	Real der_d2f_dxdx = Derivation::NSecDer2Partial(f, 0, 0, pos);  // O(h²), 3 evals
	Real der_d2f_dydy = Derivation::NSecDer4Partial(f, 1, 1, pos);  // O(h⁴), 5 evals
	// Mixed second partial: ∂²f/∂z∂x
	Real der_d2f_dzdx = Derivation::NSecDer2Partial(f, 2, 0, pos);  // O(h²), 4 evals
}
...
void Demo_Derivation_Parametric_curve_second_and_third()
{
	...
	// Second derivatives - using optimized direct formulas (3-5 function evaluations)
	VectorN<Real, 3> sec_der_f1 = Derivation::NSecDer2(f, 0.5);  // O(h²), 3 evals
	VectorN<Real, 3> sec_der_f2 = Derivation::NSecDer4(f, 0.5);  // O(h⁴), 5 evals
	VectorN<Real, 3> sec_der_f3 = Derivation::DeriveCurveSec<3>(f, 0.5, nullptr);

	// Third derivatives - using optimized direct formulas (4-6 function evaluations)
	VectorN<Real, 3> third_der_f1 = Derivation::NThirdDer2(f, 0.5);  // O(h²), 4 evals
	VectorN<Real, 3> third_der_f2 = Derivation::NThirdDer4(f, 0.5);  // O(h⁴), 6 evals
	VectorN<Real, 3> third_der_f3 = Derivation::DeriveCurveThird<3>(f, 0.5, nullptr);
}
