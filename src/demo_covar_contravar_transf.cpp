...
void Demo_velocity_contravar_transf()
{
    ...
    Vector3Spherical v_transf_to_spher = CoordTransfCartToSpher.contravariantTransf(v_cart, x1_cart);
    cout << "contravar transf. to spher at cart.pnt : " << x1_cart << " = " << v_transf_to_spher << std::endl;

    Vector3Cartesian v_back_transf_to_cart = CoordTransfSpherToCart.contravariantTransf(v_transf_to_spher, x1_spher);
...
void Demo_gradient_covariant_transf()
{
    ...
    Vector3Spherical grad_transf_to_spher = CoordTransfCartToSpher.covariantTransf(grad_cart, p_spher);
    Vector3Cartesian back_transf_to_cart = CoordTransfSpherToCart.covariantTransf(grad_transf_to_spher, p_cart);
