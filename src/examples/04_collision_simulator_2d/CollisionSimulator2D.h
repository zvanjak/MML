...
namespace Collision2D
{
    ...
    public:
        ...
        void SimulateOneStepFast(double dt, int stepNum)
        {
            ...
            // Build spatial grid
            _box.SetNumBallsInSubdividedContainer(M.rows(), M.cols(), M);
        ...
        // Parallel space subdivision - uses all available CPU cores
        void SimulateOneStepFastMultithread(double dt, int stepNum)
        {
            ...
            // Parallel position update
            {
                ...
                int chunkSize = (NumBalls + numThreads - 1) / numThreads;
                for (unsigned int t = 0; t < numThreads; t++)
                {
                    int start = t * chunkSize;
                    int end = std::min(start + chunkSize, NumBalls);
                    if (start >= NumBalls) break;
            ...
            // Build spatial grid (must be serial - assigns balls to cells)
            _box.SetNumBallsInSubdividedContainer(M.rows(), M.cols(), M);

            // Parallel collision detection - chunk grid ROWS across threads
            // Each thread processes a stripe of rows (cells in same row are independent)
            {
                std::vector<std::thread> threads;
                int totalRows = M.rows();
                int rowsPerThread = (totalRows + numThreads - 1) / numThreads;
                for (unsigned int t = 0; t < numThreads; t++)
                {
                    int rowStart = t * rowsPerThread;
                    int rowEnd = std::min(rowStart + rowsPerThread, totalRows);
                    if (rowStart >= totalRows) break;
                    threads.emplace_back([this, rowStart, rowEnd, stepNum, dt]()
                    {
