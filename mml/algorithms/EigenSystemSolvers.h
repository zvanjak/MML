...
namespace MML {
	...
	/// config.tolerance = 1e-14;       // Higher precision
	...
	struct EigenSolverConfig {
		...
		/// Sort eigenvalues in ascending order (default: true)
		/// When true, eigenvalues are sorted and eigenvectors reordered to match
		bool sort_eigenvalues = true;

		/// Compute eigenvectors (default: true)
		/// Set to false for eigenvalues-only computation (faster)
		/// Note: Not all algorithms support eigenvalues-only mode
		...
		static Result Solve(const Matrix<Real>& A, Real tol = 1e-10, int maxIter = 1000) {
			...
			for (size_t i = 0; i < result.eigenvalues.size(); i++) {
				if (!result.isComplexPair[i] || (i > 0 && result.isComplexPair[i - 1])) {
					...
					Real residual = EigenSolverHelpers::EigenvectorResidual(A, v, result.eigenvalues[i].real, result.eigenvalues[i].imag);
