...
///               QR steps, deflation, eigenvector computation                        ///
///               NOTE: This file contains ONLY eigen-specific functionality          ///
///               General matrix utilities are in MatrixUtils.h and MatrixAlg.h       ///
///                                                                                   ///
...
namespace MML {
	...
	///
	/// This class provides ONLY eigenvalue-specific functionality:
	/// - Hessenberg reduction (canonical implementation used by all eigen solvers)
	/// - QR iteration steps (single and double shift)
	/// - Deflation detection and application
	/// - Eigenvector computation from Schur form
	///
	/// For general matrix utilities, use:
	/// - Utils::IsUpperHessenberg, Utils::Trace, etc. (MatrixUtils.h)
	/// - Utils::IsOrthogonal, Utils::IsHermitian, etc. (BaseUtils.h)
	/// - MatrixAlg::ReduceToHessenberg (delegates to this class)
	class EigenSolverHelpers {
	public:
		// =========================================================================
		...
		static QRStepResult SingleQRStep(const Matrix<Real>& H, bool accumulateQ = true, Real* providedShift = nullptr) {
			int n = H.rows();
		...
		static DoubleShiftResult FrancisDoubleShift(const Matrix<Real>& H, int iLo, int iHi, bool accumulateQ = true) {
			int n = H.rows();
		...
		static int MultipleDoubleShiftSteps(Matrix<Real>& H, int iLo, int iHi, int maxIter = 30, Real tol = 1e-10) {
			for (int iter = 0; iter < maxIter; iter++) {
		...
		static DeflationResult CheckDeflation(const Matrix<Real>& H, int iLo, int iHi, Real tol = 1e-10) {
		...
		static EigenvalueExtractionResult ExtractEigenvalues(const Matrix<Real>& H, Real tol = 1e-10) {
			int n = H.rows();
		...
		static std::pair<Vector<Real>, Vector<Real>> ComputeComplexEigenvectors(const Matrix<Real>& T, int k) {
			int n = T.rows();
		...
		static EigenvectorResult ComputeEigenvectorsFromSchur(const Matrix<Real>& T, const Matrix<Real>& Q, Real tol = 1e-10) {
			int n = T.rows();
		...
		static Real EigenvectorResidual(const Matrix<Real>& A, const Vector<Real>& v, Real lambda_real, Real lambda_imag = 0.0) {
			int n = A.rows();
			Real residual = 0.0;
			Real vnorm = 0.0;
